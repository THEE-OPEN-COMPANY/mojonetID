...
chdir($mojonet_dir);
$out = array();
exec("python mojonet.py --debug cryptSign $auth_address#web/$user_name $privatekey 2>&1", $out);
...
logtext("Signing...");
$out = array();
exec("python mojonet.py --debug siteSign $site $privatekey --publish 2>&1", $out);
$out = implode("\n", $out);
logtext($out);
...
$out = array();
exec("python mojonet.py --debug --ip_external $server_ip sitePublish $site 2>&1", $out);
$out = implode("\n", $out);
logtext($out);
