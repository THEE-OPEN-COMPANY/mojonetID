class mojoID extends mojoFrame
	init: ->
		@users = {}
		@auth_address = null
		@site_info = null
		...
		$(".button-mute").on "click", =>
			...
			@searchAuthAddress val, (user_name, cert) =>
				...
				Page.cmd "muteAdd", [auth_address, val+"@mojoid.bit", ""], =>
					Page.cmd "wrapperNotification", ["done", "User #{val}@mojoid.bit muted!"]
	...
	sendRequest: ->
		...
		$.post "https://mojoid.qc.to/mojoID/request.php", {"auth_address": @auth_address, "user_name": $(".username").val(), "width": $(".ui h1").width() }, (res) =>
	...
	solveTask: (task) ->
		...
		$.post "https://mojoid.qc.to/mojoID/solution.php", {"auth_address": @auth_address, "user_name": $(".username").val(), "work_id": task.work_id, "work_solution": solution }, (res) =>
			if res == "OK"
	...
	setCert: (auth_type, user_name, cert_sign) ->
		...
		$(".current-auth b").text("#{auth_type}/#{user_name}@mojoid.bit")
