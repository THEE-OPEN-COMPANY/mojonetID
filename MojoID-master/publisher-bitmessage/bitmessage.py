...
os.chdir(config.mojonet_dir)


def log(text):
    print text
    open(config.logfile, "a").write("\n[%s] %s" % (
        time.strftime("%a, %d %b %Y %H:%M:%S"), text))


def addUser(auth_address, user_name):
    data = json.load(open("data/%s/data/users_archive.json" %
                     config.site_address))
    ...
    sign = os.popen("python mojonet.py --debug cryptSign %s#bitmsg/%s %s 2>&1" %
                    (auth_address, user_name, config.site_privatekey)).readlines()[-1].strip()
    ...
    log("Saving json")
    json.dump(data, open("data/%s/data/users.json" %
              config.site_address, "w"), indent=2, sort_keys=True)

    log("Publishing")

    res = os.popen("python mojonet.py --debug siteSign %s %s --publish 2>&1" %
                   (config.site_address, config.site_privatekey)).read()
...
for message in json.loads(res)["inboxMessages"]:
    try:
        ...
        if subject.startswith("add:"):
            ...
            log("Unknown message: %s %s %s" %
                (message["msgid"], subject, body))
            continue
